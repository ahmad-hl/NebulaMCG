...
experiments_dir_names = ['wifi_expr1','wifi_expr2','wifi_expr3','wifi_expr4','wifi_expr5']
exper_mtp_dir_names_rtc = ['wifi_expr3','wifi_expr4','wifi_expr5']
exper_rtt_dir_names_rtc = ['wifi_expr4','wifi_expr5']
...
p1 = axes[0].bar(ind, mtpMeans, width, yerr=[lowersmtp, mtpStds], log=False, capsize=16,
             color='lightgreen', error_kw=dict(elinewidth=2, ecolor='green'), label='Motion-to-Photon Latency')
...
axes[0].set_title('Eduroam WiFi', fontsize=18)
...
p3 = axes[1].bar(ind, psnrMeans, width, yerr=[lowers, psnrStds], log=False, capsize=16,
             error_kw=dict(elinewidth=3, ecolor='black'), label='PSNR')
# Optional code - Make plot look nicer
plt.margins(0.01, 0)
i = 0.12
...
fig.savefig('mtp_psnr_wifi.eps', format='eps',bbox_inches='tight')
# plt.savefig('mtp_psnr_wifi.pdf', bbox_inches='tight')
plt.show()